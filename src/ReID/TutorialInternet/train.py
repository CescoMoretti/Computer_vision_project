...
def train_model(opt, model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, use_gpu, num_epochs=25):
    ######################################################################
    # Training the model
    # ------------------
    #
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # deep copy the model
            if phase == 'val' and epoch%4 == 0: #epoch%10 == 9
                last_model_wts = model.state_dict()
                if len(opt.gpu_ids)>1:
                    save_network(model.module, epoch+1, opt.gpu_ids)
                else:
                    save_network(model, epoch+1, opt.gpu_ids)
            if phase == 'val':
                draw_curve(epoch, y_loss,y_err, name=opt.name)
            if phase == 'train':
               scheduler.step()
    ...
    if len(opt.gpu_ids)>1:
        save_network(model.module, 'last', opt.gpu_ids)
    else:
        save_network(model, 'last', opt.gpu_ids)
...
def draw_curve(current_epoch, y_loss, y_err, name):
    ...
    if current_epoch == 0:
        ax0.legend()
        ax1.legend()
    fig.savefig(os.path.join('C:\Computer_vision_project\src\ReID\DeepReID\Result','train.jpg'))

######################################################################
# Save model
#---------------------------
def save_network(network, epoch_label, gpu_ids):
    save_filename = 'net_%s.pth'% epoch_label
    save_path = os.path.join('C:\Computer_vision_project\src\ReID\DeepReID\Result',save_filename)
...
def main():
    ...
    dir_name = "C:\Computer_vision_project\src\ReID\TutorialInternet\Result"
    # save opts
    ...
    model = train_model(opt=opt, model=model, criterion=criterion, optimizer=optimizer_ft, scheduler=exp_lr_scheduler,
                        dataloaders=dataloaders, dataset_sizes=dataset_sizes, use_gpu=use_gpu, num_epochs=opt.total_epoch)

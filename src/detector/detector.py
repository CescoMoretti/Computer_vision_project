...
class Detector():
    ...
    def detect(self, frame):
        result = self.model.predict(frame, device="cpu", classes=[0])[0] #this is to pass detection of the first frame (we only give one)
        return result.boxes.cpu().numpy().cls, result.boxes.cpu().numpy().conf, result.boxes.cpu().numpy().xyxy

    def draw_resut(self, frame, labels, boxes, color):
        for (labe, box) in zip( labels, boxes):
